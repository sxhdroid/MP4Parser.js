...
//             ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -> BoxData
//
function _parse(view, res, path, boxHeadOffset) {
...
function _moov(boxView, res, path) {
    ...
    _parse(boxView, res, path, boxView.boxHead);
}
...
function _trak(boxView, res, path) {
    ...
    //                           ^^
    _parse(boxView, res, path + ":" + (r.length - 1), boxView.boxHead);
}
...
//}@dev
    ...
    for (var i = 0; i < entry_count; ++i) {
        _parse(boxView, res, path, boxView.boxHead); // call _url_
    }
}
...
//}@dev
    ...
    _parse(boxView, res, path, boxView.boxHead); // _avcC
}
