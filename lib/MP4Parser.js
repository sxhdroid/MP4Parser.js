...
- RBSP
    - SODB + RBSP Stop Bit + 0bit(s)
- EBSP
    - RBSP(0) + 0x03 + RBSP(1) + 0x03 ... RBSP(N)
- NALU
    - NALUHeader + EBSP
- H264 ByteStream
    ...
    - https://codesequoia.wordpress.com/2009/10/18/h-264-stream-structure/
    - http://gentlelogic.blogspot.jp/2011/11/exploring-h264-part-2-h264-bitstream.html
...
- DiagnosticInformationObject
    - detail: ObjectArray
        ...
        - BoxPath: String - "moov/trak:0/mdia/minf/stbl/stco"
...
var MP4Parser = {
    ...
    "parse_mdat":   MP4Parser_parse_mdat,   // MP4Parser.parse_mdat(source:Uint8Array):NALUnitArray
    "repository":   "https://github.com/uupaa/MP4Parser.js",
};
...
function _ftyp(boxView, tree, diagnostic, path) {
    ...
    _box(tree, diagnostic, path, {
        ...
        "BoxType":  "ftyp",
        "BoxPath":  path,
        "major_brand":       major_brand,       // "isom"
        "minor_version":     minor_version,     // 512
        "compatible_brands": compatible_brands, // ["isom", "iso2", "avc1", "mp41"]
    });
}
...
function _hdlr(boxView, tree, diagnostic, path) {
    ...
    var handler_type    = _read4(boxView);          // mdia/hdlr.handler_type = 1986618469 = 0x76696465 = "vide"
                                                    // meta/hdlr.handler_type = 1835297138 = 0x6d646972 = "mdir"
    var handler_type2   = _read4(boxView);          // mdia/hdlr.handler_type2 = 0
                                                    // meta/hdlr.handler_type2 = 1634758764 = 0x6170706c = "appl"
    var reserved2       = _read4(boxView);          // = 0
    var reserved3       = _read4(boxView);          // = 0
    var name            = [];
    var c;
...
function _stss(boxView, tree, diagnostic, path) {
    // Sync Sample Box. Random access table. optional.
    // ランダム・アクセス可能なサンプル番号(フレーム番号)のテーブル
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
    ...
    for (var i = 0; i < entry_count; ++i) {
        samples.push({ "sample_number": _read4(boxView) }); // frame number
    }
    _box(tree, diagnostic, path, {
        "BoxHead":      boxView.boxHead,
        "BoxSize":      boxView.source.length,
...
function MP4Parser_parse_mdat(source) { // @arg Uint8Array - mdat binary data
