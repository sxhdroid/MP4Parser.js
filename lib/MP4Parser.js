...
// --- class / interfaces ----------------------------------
var MP4Parser = {
    "parse":    MP4Parser_parse,      // MP4Parser.parse(source:Uint8Array, cursor:UINT32 = 0):MP4BoxTreeObject
    "mdat": {
...
function MP4Parser_parse(source,   // @arg Uint8Array
                         cursor) { // @arg UINT32 = 0
                                   // @ret MP4BoxTreeObject - { ftyp, mdat, moov, ... }
    ...
    _parse(view, mp4boxTreeObject, "root");
...
//             ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -> BoxData
//
function _parse(view, tree, path, boxHeadOffset) {
    ...
    while (view.cursor < view.source.length) {
        ...
        switch (boxType) {
        case "ftyp": _ftyp(boxView, tree, path + "/ftyp"); break;
        case "moov": _moov(boxView, tree, path + "/moov"); break;
        case "mdat": _mdat(boxView, tree, path + "/mdat"); break;
        case "free": _free(boxView, tree, path + "/free"); break;
        case "mvhd": _mvhd(boxView, tree, path + "/mvhd"); break;
        case "trak": _trak(boxView, tree, path + "/trak"); break; // array
        case "tkhd": _tkhd(boxView, tree, path + "/tkhd"); break;
        case "edts": _edts(boxView, tree, path + "/edts"); break;
        case "elst": _elst(boxView, tree, path + "/elst"); break;
        case "mdia": _mdia(boxView, tree, path + "/mdia"); break;
        case "mdhd": _mdhd(boxView, tree, path + "/mdhd"); break;
        case "hdlr": _hdlr(boxView, tree, path + "/hdlr"); break;
        case "minf": _minf(boxView, tree, path + "/minf"); break;
        case "vmhd": _vmhd(boxView, tree, path + "/vmhd"); break;
        case "dinf": _dinf(boxView, tree, path + "/dinf"); break;
        case "dref": _dref(boxView, tree, path + "/dref"); break;
        case "url ": _url_(boxView, tree, path + "/url "); break; // array
        case "stbl": _stbl(boxView, tree, path + "/stbl"); break;
        case "stsd": _stsd(boxView, tree, path + "/stsd"); break;
        case "stts": _stts(boxView, tree, path + "/stts"); break;
        case "stss": _stss(boxView, tree, path + "/stss"); break;
        case "stsc": _stsc(boxView, tree, path + "/stsc"); break;
        case "stsz": _stsz(boxView, tree, path + "/stsz"); break;
        case "stco": _stco(boxView, tree, path + "/stco"); break;
        case "udta": _udta(boxView, tree, path + "/udta"); break;
        case "meta": _meta(boxView, tree, path + "/meta"); break;
        case "ilst": _ilst(boxView, tree, path + "/ilst"); break;
        case "avc1": _avc1(boxView, tree, path + "/avc1"); break; // [AVC extend]
        case "avcC": _avcC(boxView, tree, path + "/avcC"); break; // [AVC extend]
...
//  }
function _ftyp(boxView, tree, path) {
...
//  }
function _moov(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "moov",
    });
    _parse(boxView, tree, path, boxView.boxHead);
}

//  aligned(8) class MediaDataBox extends Box('mdat') {
//      bit(8) data[];
//  }
function _mdat(boxView, tree, path) {
/*
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
 ...
 */
    ...
    _box(tree, path, handler);
}

//  aligned(8) class FreeSpaceBox extends Box(free_type) {
//      unsigned int(8) data[];
//  }
function _free(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "free",
...
//  }
function _mvhd(boxView, tree, path) {
...
//  }
function _trak(boxView, tree, path) {
    //  moov: {
    //      track: [
    //          videoTrack,
    //          audioTrack
    ...
    //                           ^^
    _parse(boxView, tree, path + ":" + (r.length - 1), boxView.boxHead);
}
...
//  }
function _tkhd(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
...
//  }
function _edts(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "edts",
...
//      }
//  }
function _elst(boxView, tree, path) {
...
//  }
function _mdia(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "mdia",
...
//  }
function _mdhd(boxView, tree, path) {
    var version             = _read1(boxView);
    var flags               = _read3(boxView);
...
//      string name;
//  }
function _hdlr(boxView, tree, path) {
    var version         = _read1(boxView);          // = 0
    var flags           = _read3(boxView);          // = 0
...
//  }
function _minf(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "minf",
...
//  }
function _vmhd(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
...
//  }
function _dinf(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "dinf",
...
//      }
//  }
function _dref(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
...
//}@dev
    ...
    for (var i = 0; i < entry_count; ++i) {
        _parse(boxView, tree, path, boxView.boxHead); // call _url_
    }
}
...
//      string location;
//  }
function _url_(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
...
//  }
function _stbl(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "stbl",
...
//      }
//  }
function _stsd(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
...
//     }
//  }
function _stts(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
...
//      }
//  }
function _stss(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
...
//      }
//  }
function _stsc(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
...
//      }
//  }
function _stsz(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
...
//      }
//  }
function _stco(boxView, tree, path) {
    var version         = _read1(boxView); // = 0
    var flags           = _read3(boxView); // = 0
...
function _udta(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "udta",
...
function _meta(boxView, tree, path) {
    var version         = _read1(boxView);
    var flags           = _read3(boxView);
...
function _ilst(boxView, tree, path) {
    _box(tree, path, {
        "BoxHead":  boxView.boxHead,
        "BoxSize":  boxView.source.length,
        "BoxType":  "ilst",
...
//  }
function _avc1(boxView, tree, path) {
...
//}@dev
    ...
    _parse(boxView, tree, path, boxView.boxHead); // _avcC
}
...
//      }
//  }
function _avcC(boxView, tree, path) {
